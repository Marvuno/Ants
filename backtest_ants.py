...
def ants_indicator(tickers:list) -> None:
  main_df = yq.Ticker(tickers, formatted=True, validate=True).history(period="3y", interval="1d")
  ...
  for n, ticker in enumerate(tickers):
    ...
    try:
        if len(new_indexes[3]) > 0 and len(df) >=500:
            if min(new_indexes[3])+15 < 500:
                buy_signal = "[BUY]"
                buy_list.append([ticker, 
                                df['date'][min(new_indexes[3])+15].strftime('%Y-%m-%d'),
                                round(df['adjclose'][min(new_indexes[3])+15], 4), 
                                round(df['adjclose'][min(new_indexes[3])+15+252], 4)]) # ticker, 1y ago and today price
    except:
      pass
